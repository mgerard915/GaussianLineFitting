...
def full_line_model(x, A_hb, A_oiii, A_ha, mu_ha, R_nii, R_sii,
    ...
    model = (
        ...
        + gaussian(x, A_sii, mu_sii_6731, sig_sii_6731)
        + m * x + b
    )
    return model
...
def detect_line_snr(wave, spec, err, center, R_interp, n_sigma_window=3):
    ...
    sb_mask = ((wave > center - 6 * sig) & (wave < center - 3 * sig) |
...
def initial_fits(wave, spectrum, err_spec, window, hb_center, oiii_center, ha_center,
    ...
    sigma_int_lo = sigma_inst_at_ha * 0.1
    sigma_int_hi = sigma_inst_at_ha * 2
    ...
    R_nii_lo, R_nii_hi = (0.0, 3.0) if fit_nii else (0.0, 1e-8)
    R_sii_lo, R_sii_hi = (0.0, 2.0) if fit_sii else (0.0, 1e-8)
...
def line_fitting(wave, flux, flux_err, R_interp, hb_center=0.4867, oiii_center=0.5007, ha_center=0.6563,
    ...
    df['sigma_sii_broad'] = np.sqrt(np.maximum(df['sigma_int'] ** 2 + inst_sigma(mu_sii_s, R_interp) ** 2, 1e-20))
    ...
    df['Flux_SII'] = df['A_sii'] * df['sigma_sii_broad'] * np.sqrt(2 * np.pi)
